...
try {
 ...
 try {
      ...
      return const Left(CacheFailure(message: 'لم نتمكن من تغيير حالة العادة.'));
 }
  }

  @override
  Stream<Either<Failure, List<HabitEntity>>> watchHabits() {
   return localDataSource.getCachedHabits().map<Either<Failure, List<HabitEntity>>>(
      (habitModels) {
        try {
          // بنحول كل Model لـ Entity عشان نطلعه للـ Domain والـ UI
          final entities = habitModels.map((model) => model.toEntity()).toList();
          return Right(entities);
        } catch (e) {
          return const Left(CacheFailure(message: 'خطأ أثناء قراءة العادات من الكاش.'));
        }
      },
    );
  }
